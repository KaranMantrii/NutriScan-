...
import { User, Target, ShieldAlert, Salad, ArrowRight, ArrowLeft, Pencil, House, ScanLine, TrendingDown, TrendingUp, AlertTriangle, Zap, Scale, Heart, Moon, Activity, Leaf, Check, Utensils } from "lucide-react";
...
const ACTIVITY_LEVELS = [
    ...
    { value: "moderate",    label: "Moderately Active", sub: "3–5 workouts per week" },
    { value: "active",      label: "Very Active",       sub: "Hard exercise 6–7 days/week" },
    { value: "athlete",     label: "Athlete",           sub: "Twice daily / physical job" },
];

const HEALTH_GOALS = [
    { label: "Weight Loss",         icon: TrendingDown, iconColor: "text-amber-400",  iconBg: "bg-amber-950/70",  desc: "Penalises high-calorie and high-sugar products." },
    { label: "Muscle Gain",         icon: Zap,          iconColor: "text-green-400",  iconBg: "bg-green-950/70",  desc: "Rewards protein-dense foods." },
    { label: "Maintain Weight",     icon: Scale,        iconColor: "text-blue-400",   iconBg: "bg-blue-950/70",   desc: "Balances calories in vs. out." },
    { label: "Heart Health",        icon: Heart,        iconColor: "text-green-400",  iconBg: "bg-green-950/70",  desc: "Flags saturated fat and sodium more heavily." },
    { label: "Diabetes Management", icon: Activity,     iconColor: "text-red-400",    iconBg: "bg-red-950/70",    desc: "Triples the sugar penalty in scoring." },
    { label: "Improve Energy",      icon: Zap,          iconColor: "text-yellow-400", iconBg: "bg-yellow-950/70", desc: "Prioritises complex carbs and iron-rich foods." },
    { label: "Better Sleep",        icon: Moon,         iconColor: "text-purple-400", iconBg: "bg-purple-950/70", desc: "Flags caffeine and high-sugar products." },
    { label: "Build Endurance",     icon: Activity,     iconColor: "text-cyan-400",   iconBg: "bg-cyan-950/70",   desc: "Boosts scores for complex carb-rich foods." },
    { label: "Reduce Cholesterol",  icon: Heart,        iconColor: "text-rose-400",   iconBg: "bg-rose-950/70",   desc: "Penalises saturated and trans fats heavily." },
    { label: "Gut Health",          icon: Leaf,         iconColor: "text-lime-400",   iconBg: "bg-lime-950/70",   desc: "Rewards fibre-rich and probiotic foods." },
];
...
function ActivityRow({ label, sub, selected, onClick }) {
    return (
        ...
        >
            ...
            <span className={`text-xs ${selected ? "text-green-400" : "text-zinc-500"}`}>{sub}</span>
        </button>
    );
}
    ...
    return (
        <div className="space-y-5">
            <div className="bg-white/5 border border-white/5 rounded-3xl p-5 space-y-5">
                ...
                {/* ACTIVITY LEVEL */}
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-semibold text-white">Activity Level</label>
...
function StepHealthGoals({ profile, toggleArrayField }) {
    return (
        <div className="space-y-3">
            {HEALTH_GOALS.map(({ label, icon: GoalIcon, iconColor, iconBg, desc }) => {
                ...
                return (
                    <button
                        ...
                        className={`w-full flex items-center gap-4 p-4 rounded-2xl border text-left transition-all active:scale-[0.98] ${
                            selected ? "bg-green-400/5 border-green-400" : "bg-white/[0.03] border-white/10"
                        }`}
                    >
        ...
        <div className="space-y-4">
            <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
                {ALLERGENS.map((allergen, i) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${selected ? "bg-green-400" : "bg-white/20"}`} />
                            <span className={`flex-1 text-sm font-medium ${selected ? "text-white" : "text-zinc-300"}`}>
                                {allergen}
                            </span>
                            <div className={`w-6 h-6 rounded-full border-2 flex-shrink-0 transition-all ${
                                selected ? "border-green-400 bg-green-400/20" : "border-white/20"
                            }`} />
                        </button>
                    );
                })}
            ...
            <div className="flex gap-3 p-4 rounded-2xl border border-amber-400/30 bg-amber-400/5">
                ...
                <p className="text-xs text-zinc-400 leading-relaxed">
                    Allergen data comes from Open Food Facts. Always read the physical product label as the primary safety check.
                </p>
            </div>
        </div>
    );
                ...
                return (
                    <button
                        ...
                        className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl border text-left transition-all active:scale-[0.98] ${
                            selected ? "bg-green-400/5 border-green-400/50" : "bg-white/[0.03] border-white/10"
                        }`}
                    >
...
function BMICard({ height, weight }) {
    ...
    if (bmiRaw < 18.5) {
        label = "Underweight";     textColor = "text-blue-400";   borderColor = "border-blue-400/60";  bgColor = "bg-blue-400/10";   Icon = TrendingDown;
        desc  = "You may benefit from a calorie-rich, nutrient-dense diet.";
    } else if (bmiRaw < 25) {
        label = "Normal Weight";   textColor = "text-green-400";  borderColor = "border-green-400/60"; bgColor = "bg-green-400/10";  Icon = Check;
        desc  = "You're within a healthy weight range — keep it up!";
    } else if (bmiRaw < 30) {
        label = "Overweight";      textColor = "text-amber-400";  borderColor = "border-amber-400/60"; bgColor = "bg-amber-400/10";  Icon = TrendingUp;
        desc  = "Slightly above the healthy range. A balanced diet can help.";
    } else if (bmiRaw < 35) {
        label = "Obese (Class I)"; textColor = "text-orange-400"; borderColor = "border-orange-400/60";bgColor = "bg-orange-400/10"; Icon = AlertTriangle;
        desc  = "Health risks are elevated. Consider consulting a doctor.";
    } else {
        label = "Obese (Class II+)";textColor = "text-red-400";  borderColor = "border-red-400/60";   bgColor = "bg-red-400/10";    Icon = AlertTriangle;
    ...
    return (
        <div className="bg-white/5 border border-white/5 rounded-2xl p-5">
            <p className="text-xs text-zinc-500 uppercase tracking-wider mb-4">Body Mass Index</p>

            <div className="flex items-center gap-4 mb-5">
                {/* Score circle */}
                <div className={`w-20 h-20 rounded-full border-[3px] flex flex-col items-center justify-center flex-shrink-0 ${borderColor} ${bgColor}`}>
                    <p className={`text-2xl font-bold leading-none ${textColor}`}>{bmi}</p>
                    <p className="text-[9px] text-zinc-500 uppercase tracking-widest mt-0.5">BMI</p>
                </div>
                {/* Category + description */}
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1">
                        <Icon className={`w-4 h-4 flex-shrink-0 ${textColor}`} />
                        <p className={`font-bold text-base leading-tight ${textColor}`}>{label}</p>
                    </div>
            ...
            <div className="relative">
                ...
                <div className="flex justify-between mt-1.5 text-[9px] text-zinc-500 leading-tight">
                    ...
                    <span className="text-center">Normal<br/>18.5–25</span>
                    <span className="text-center">Over<br/>25–30</span>
...
function ProfileView({ profile, onEdit }) {
    const ACTIVITY_LABEL = ACTIVITY_LEVELS.find(a => a.value === profile.activityLevel)?.label || "—";
    const ACTIVITY_SUB   = ACTIVITY_LEVELS.find(a => a.value === profile.activityLevel)?.sub   || "";
            ...
            <div className="px-5 space-y-4 overflow-y-auto">
                ...
                <div className="flex flex-col items-center py-6 gap-3">
                    ...
                    <div className="text-center">
                        <p className="text-2xl font-bold text-white">{profile.name || "No name set"}</p>
                        <p className="text-sm text-zinc-400 mt-0.5 capitalize">{profile.gender || ""}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                    {[
                        ...
                        <div key={label} className="bg-white/5 border border-white/5 rounded-2xl p-4 flex flex-col items-center gap-1">
                ...
                <div className="bg-white/5 border border-white/5 rounded-2xl p-4 flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Activity Level</p>
                        <p className="text-white font-semibold text-sm mt-0.5">{ACTIVITY_LABEL || "—"}</p>
                        {ACTIVITY_SUB && <p className="text-[11px] text-zinc-500 mt-0.5">{ACTIVITY_SUB}</p>}
                    </div>
                </div>
                ...
                {profile.healthGoals.length > 0 && (
                    <div className="bg-white/5 border border-white/5 rounded-2xl p-4">
                        <p className="text-xs text-zinc-500 uppercase tracking-wider mb-3">Health Goals</p>
                        <div className="flex flex-col gap-2">
                            {profile.healthGoals.map(g => {
                                ...
                                return (
                                    <div key={g} className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm text-white font-medium leading-tight">{g}</p>
                ...
                {profile.allergens.length > 0 && (
                    <div className="bg-white/5 border border-white/5 rounded-2xl p-4">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <p className="text-xs text-zinc-500 uppercase tracking-wider">Allergens</p>
                        </div>
                ...
                {profile.dietaryPreferences.length > 0 && (
                    <div className="bg-white/5 border border-white/5 rounded-2xl p-4">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <p className="text-xs text-zinc-500 uppercase tracking-wider">Dietary Preferences</p>
                        </div>
                        <div className="flex flex-wrap gap-2">
...
export default function Profile() {
    ...
    const { profile, updateField, toggleArrayField, saveProfile, loadProfile } = useProfile();
    ...
    if (isSetupDone) {
        return <ProfileView profile={profile} onEdit={() => { setIsSetupDone(false); setCurrentStep(1); }} />;
    }
    ...
    return (
        <div className="min-h-dvh text-white flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-32">
                {currentStep === 1 && <StepAboutYou profile={profile} updateField={updateField} />}
                {currentStep === 2 && <StepHealthGoals profile={profile} toggleArrayField={toggleArrayField} />}
                {currentStep === 3 && <StepAllergens profile={profile} toggleArrayField={toggleArrayField} />}
                {currentStep === 4 && <StepDiet profile={profile} toggleArrayField={toggleArrayField} />}
            </div>
