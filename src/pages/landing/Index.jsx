    ...
    <BrowserRouter>
      <div className="w-full min-h-dvh bg-hero text-white overflow-x-hidden font-outfit">
        ...
        <main className="max-w-7xl mx-auto px-4 pt-13">
          ...
          <GlassSurface brightness={50} opacity={0.9} width="100%" height="auto" borderRadius={24}>
              ...
              <div className="flex items-center gap-2 font-stardom">
                ...
                <section className="relative z-10 py-16 px-4 max-w-7xl mx-auto">
                  <div className="text-center mb-12">
                    <span className="text-green-400 font-chillax text-xs uppercase tracking-[0.2em] mb-2 block">Core Capabilities</span>
                    ...
                    <p className="text-zinc-500 font-outfit max-w-md mx-auto mt-1 text-sm">
                      NutriScan+ transforms raw barcode data into actionable health insights.
                    </p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <FeatureCard icon={<Zap />} title="Instant Scan" tag="LIVE" desc="Our proprietary 3-layer verification scans barcodes and cross-references global databases in under 0.8 seconds." />
                    <FeatureCard icon={<Salad />} title="Clean Label" tag="SMART" desc="Instantly identify ultra-processed ingredients, hidden sugars, and harmful additives with a simple color grade." />
                    <FeatureCard icon={<Dumbbell />} title="Custom Macro" tag="PRO" desc="Set your fitness goals and let Nutriscan+ calculate if a product fits your daily protein and calorie windows." />
                  </div>
                </section>
                ...
                <section className="relative z-10 py-16 px-4 max-w-3xl mx-auto">
                  <div className="text-center mb-10">
                       <h2 className="text-4xl sm:text-4xl font-bold mb-2 font-author py-1">
                         <GradientText colors={["#ffffff", "#84CC16"]}>How it Works</GradientText>
                       </h2>
                       <p className="text-zinc-500 font-outfit text-sm">Three simple steps to nutritional clarity.</p>
                    </div>
                    
                    <div className="bg-white/2 border border-white/5 rounded-3xl p-6 backdrop-blur-3xl overflow-hidden">
                      <Stepper initialStep={1}>
                        <Step title="1. Aim"><p className="text-zinc-400 text-base font-light font-outfit">Click the get started button to navigate to the home page, then hold your mobile phone steady and place the product barcode in the barcode scanning frame.</p></Step>
                        <Step title="2. Analyze"><p className="text-zinc-400 text-base font-light font-outfit">The system will immediately lock onto the tracking lines, decipher the manufacturing data, and cross-reference massive nutritional networks in a split second.</p></Step>
                        <Step title="3. Choose"><p className="text-zinc-400 text-base font-light font-outfit">You can then review how the item impacts your personal daily goals, explore healthier alternative items recommended by our analysis, and confidently make the best decision for your lifestyle without guessing.</p></Step>
                      </Stepper>
                    </div>
                </section>
              </>
...
function FeatureCard({ icon, title, tag, desc }) {
  return (
    <ElectricBorder color="#84CC16" speed={0.15} chaos={0.05} thickness={2} style={{ borderRadius: 24 }}>
      <div className="h-full p-6 rounded-[22px] bg-white/3 backdrop-blur-2xl border border-white/5 flex flex-col group transition-all duration-300">
        ...
        <div className="flex items-center gap-3 mb-3 font-author">
          ...
          <span className="text-[9px] font-black tracking-widest bg-white/10 px-1.5 py-0.5 rounded text-zinc-300 uppercase">{tag}</span>
        </div>
        <p className="text-zinc-400 text-sm leading-relaxed font-light font-outfit">{desc}</p>
      </div>
    </ElectricBorder>
  );
}
